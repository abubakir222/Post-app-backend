...
const notificationCtrl = {
  ...
  getNotifications: async (req, res) => {
    try {
      const userId = req.user.id; // userId ni token orqali olish
      if (!userId) return res.status(400).json({ message: 'User ID majburiy!' });
  ...
  createNotification: async (req, res) => {
    try {
      ...
      if (!sender || !receiver) return res.status(404).json({ message: 'Sender yoki receiver topilmadi!' });
      ...
      if (!notifMessage) {
        if (type === 'comment') notifMessage = `${sender.username || sender.name || 'User'} sizning postingizga komment yozdi: "${postContent.substring(0, 20)}..."`;
        else if (type === 'like') notifMessage = `${sender.username || sender.name || 'User'} sizning postingizga like bosdi: "${postContent.substring(0, 20)}..."`;
        else if (type === 'follow') notifMessage = `${sender.username || sender.name || 'User'} sizni kuzatishni boshladi`;
        else notifMessage = `Yangi xabar: ${type}`;
      }
      ...
      if (receiverSocketId && global._io) {
